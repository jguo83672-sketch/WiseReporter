...
def edit_report(report_id):
    ...
    if request.method == 'POST':
        ...
        if period_start:
            ...
            report.period_start = datetime.strptime(period_start, '%Y-%m-%d').date()
        if period_end:
            from datetime import datetime
            report.period_end = datetime.strptime(period_end, '%Y-%m-%d').date()
...
def generate_report():
    ...
    if request.method == 'POST':
        ...
        report = generator.generate_report(start_date=start_date, end_date=end_date)
        return redirect(url_for('main.report_detail', report_id=report.id))
...
def cookies():
    ...
    for c in cookies_list:
        ...
        cookies_data.append({
            ...
            'cookie_data': c.cookie_data,  # 编辑时需要用到
            'wxuin': wxuin,
